...
$str=basename($_SERVER['PHP_SELF']);
        $tt=explode(".",$str);
$stm = $bdd->prepare("SELECT *  FROM produit WHERE type=?");
$stm->execute([$tt[0]]);
?>
<html>
<head>
...
<div class="banner">
    ...
    <!-- cart's-Product-Display -->
      ...
      <?php
         while ($row = $stm->fetch()) {
			$st = $bdd->prepare("SELECT *  FROM produit WHERE nom=?");
			$st->execute([$row['nom']]);
			$qt = $st->rowCount();
              ...
              <form action='#' method='post'>
				...
				if(isset($_SESSION['role'])){
					if($_SESSION['role'] === "admin"){
						...
						echo  "<button type='submit' name='add' class='button w3l-cart' data-id='cart-1'>add to cart</button>";
						}
				}
